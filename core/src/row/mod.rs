...
/// A trait specifying single `Row` of [`Bell`]s.
///
...
pub trait RowTrait:
    Sized + Clone + Eq + Hash + Debug + Display + Not<Output = Self> + Mul<Self, Output = Self>
{
    ...
    /// # Example
    /// ```
    /// use proj_core::{Bell, Row, RowTrait, Stage, IncompatibleStages};
    ///
    ...
    /// # Ok::<(), InvalidRowError>(())
    /// ```
    fn parse(s: &str) -> Result<Self, InvalidRowError> {
    ...
    /// # Ok::<(), InvalidRowError>(())
    /// ```
    fn parse_with_stage(s: &str, stage: Stage) -> Result<Self, InvalidRowError> {
    ...
    /// ```
    fn backrounds(stage: Stage) -> Self {
    ...
    fn from_iter(iter: impl Iterator<Item = Bell>) -> Result<Self, InvalidRowError> {
        // This unsafety is OK because the resulting row is never used for anything other than a
        // validity check
    ...
    /// # Ok::<(), proj_core::InvalidRowError>(())
    /// ```
    fn closure_from_rounds(&self) -> Vec<Self> {
    ...
    // QUESTION: Should these be stand-alone functions? I think not
    ...
    fn is_group<'a>(rows: impl IntoIterator<Item = &'a Self>) -> Result<bool, IncompatibleStages>
    where
        Self: 'a,
    {
    ...
    /// # Ok::<(), proj_core::InvalidRowError>(())
    /// ```
    fn mul(&self, rhs: &Self) -> Result<Self, IncompatibleStages> {
    ...
    #[inline]
    fn tranposition_to(&self, other: &Self) -> Result<Self, IncompatibleStages> {
        other.mul(&self.inv())
    }
    ...
    #[inline]
    unsafe fn tranposition_to_unchecked(&self, other: &Self) -> Self {
